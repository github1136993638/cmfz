...
<script>
    $(function () {
        $("#albumList").jqGrid({
            ...
            datatype: "json",
            colNames: ["id", "标题", "分数", "作者", "播音员", "章节数", "专辑简介", "发行时间", "状态", "封面"],
            colModel: [
    ...
    function addSubGrid(subGridId, albumId) {//subGridId:子表格的id  albumId：当前子表所属专辑的id
        ...
        $("#" + tableId).jqGrid({
            ...
            datatype: "json",
            colNames: ["id", "标题", "大小", "时长", "状态", "创建日期", "音频"],
            colModel: [
                {name: "id", align: "center", width: "120px"},
                {name: "title", editable: true, align: "center", width: "80px"},
                {name: "size", align: "center", width: "50px"},//大小
                {name: "duration", align: "center", width: "50px"},//时长
                {
                    name: "status", editable: true, edittype: "select", align: "center", width: "50px",
                    editoptions: {value: "展示:展示;不展示:不展示"}
                },
                {name: "create_date", align: "center", width: "50px"},
                //{name:"aid",align: "center"},
                //音频上传到服务器并在线播放    原文链接：https://blog.csdn.net/zhaihao1996/article/details/99655758
                {
                    name: "audio", editable: true, edittype: "file", align: "center",
                    formatter: function (cellvalues, options, rowObject) {
                        //cellvalues-当前字段的值?在远程匹配到的数据 options-表格中的所有属性?当前单元格操作属性对象 rowObject-当前行的数据对象
                        //函数的返回值会显示在当前单元格
                        /*return "<a href=''> <span class='glyphicon glyphicon-play'></span></a>"+"     "+
                            "<a href=''> <span class='glyphicon glyphicon-circle-arrow-down'></span></a>"*/
                        return "<audio controls loop>\n" +
                            "  <source src=\"${cmfz}/upload/audio/" + cellvalues + "\" type=\"audio/mpeg\">\n" +
                            "</audio>";
                        /*return "<audio controls loop>\n" +
                            "  <source src=\"/upload/audio/"+cellvalues+"\" type=\"audio/mpeg\">\n" +
                            "</audio><a href='/chapter/download?id="+rowObject.id+"'> <span class='glyphicon glyphicon-circle-arrow-down'></span></a>";*/
                    }
                },
            ],
            styleUI: "Bootstrap",
